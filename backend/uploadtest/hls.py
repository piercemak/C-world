"""Signed HLS manifest support for the single-title pilot.
...
from .catalog import CatalogError, find_media
...
def build_hls_playback_payload(media):
    ...
    token = _make_token(media["id"], expires)
    manifest = str(playback.get("master") or "master.m3u8").lstrip("/")
    url = (
        f"{settings.CWORLD_PUBLIC_API_URL}/api/playback/hls/"
        f"{media['id']}/{quote(manifest, safe='/')}?token={quote(token)}"
    )
    return {
        "mediaId": media["id"],
        "season": None,
...
def _rewrite_uri(uri, media, manifest_path, token, expires):
    ...
    if resolved.lower().endswith(".m3u8"):
        return f"{settings.CWORLD_PUBLIC_API_URL}/api/playback/hls/{media['id']}/{quote(resolved, safe='/')}?token={quote(token)}"
    ...
    for line in str(text).splitlines():
        ...
        if stripped.startswith("#EXT-X-MEDIA:") or stripped.startswith("#EXT-X-I-FRAME-STREAM-INF:"):
            if "URI=\"" in line:
                ...
                line = f'{prefix}URI="{_rewrite_uri(uri, media, manifest_path, token, expires)}"{suffix}'
...
def hls_playback_session(request):
    ...
    if not media or media.get("type") != "movie":
        return Response({"error": "HLS pilot media was not found"}, status=status.HTTP_404_NOT_FOUND)
    playback = media.get("playback") or {}
    if playback.get("type") != "hls":
        return Response({"error": "This title is not enabled for HLS"}, status=status.HTTP_404_NOT_FOUND)
