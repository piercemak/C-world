...
const MobileUser = () => {
  ...
  const { activeProfile, user, logout } = useAuth();
  ...
  const [profileImage, setProfileImage] = useState(() => 
    localStorage.getItem('profileImage') || localStorage.getItem('userProfileImage') || DEFAULT_IMAGE
  );
 const isDefaultImage = profileImage === DEFAULT_IMAGE;
  useEffect(() => {
    const stored = localStorage.getItem('profileImage') || localStorage.getItem('userProfileImage');
    ...
    <div className='w-full h-dvh flex flex-col bg-black'>
          ...
          <div className='flex flex-row items-end'>
            <span className='flex items-center justify-center w-32 h-10 rounded-full bg-white p-4 gap-2'>
              ...
              <span className='text-lg font-semibold'>
                {activeProfile?.name || user?.username || "Profile"}
              </span>
            </span>
          </div>
        </motion.div>       
      ...
      />
        <motion.button
          ...
          className="mt-6 flex self-center justify-center items-center w-24 h-8 rounded-full bg-white p-4 gap-2 shadow-lg text-md font-bold z-20 cursor-pointer"
        >
          Log out
        </motion.button>
    </div>
