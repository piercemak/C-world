...
const User = () => {
  ...
  const { activeProfile, user, logout, updateActiveProfile } = useAuth();
  ...
  useEffect(() => {
    setProfileImage(activeProfile?.avatar_url || DEFAULT_IMAGE);
  ...
  const handleImageChange = async (e) => {
    ...
    reader.onloadend = async () => {
      ...
      try {
        await updateActiveProfile?.({ avatar_url: result });
      } catch (err) {
  ...
  const handleResetImage = async () => {
    ...
    try {
      await updateActiveProfile?.({ avatar_url: "" });
    } catch (err) {
